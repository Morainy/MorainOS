/*************************************************************************
    ...
    > Created Time: 2015年04月03日 星期五 21时34分16秒
 ************************************************************************/
...
struct multiboot_t{
	...
	/**
	 *从BIOS 获知的可用内存
	 *mem_lower 和mem_upper 分别指出了低端和高端内存的大小，单位是K
	 *低端地址的首地址是0 ， 高端内存的首地址是1M
	 *低端内存的最大可能值是640K
	 *高端内存的最大可能值减去1M。但并不保证是这个值。
	 */
	uint32_t mem_lower;
	uint32_t mem_upper;
	...
	/**
	 *ELF 格式内核映像的section头表。包括每项的大小， 一共有几项作为名字索引
	 *的字符串
	 */
	uint32_t num;
	uint32_t size;
	 ...
	 *缓冲区是由一个或多个下面的mmap_entry_t 组成
	 */
	uint32_t mmap_addr;
	uint32_t mmap_length;

	uint32_t drives_length;  //指出第一个驱动器结构的大小
	uint32_t drives_addr;   //指出第一个驱动器的地址
	uint32_t config_table;  //ROM配置表
	uint32_t boot_loader_table;   //boot_loader的名字
